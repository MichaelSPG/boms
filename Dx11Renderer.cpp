...
Dx11Renderer::Dx11Renderer(HWND hWnd, int renderWindowWidth, int renderWindowHeight)
	: mSwapChain(nullptr)
	, mDevice(nullptr)
	...
	, mDepthStencilView(nullptr)
{
	...
	{
		Log::logMessage("Failed to create D3D11 device", pantheios::SEV_CRITICAL);
	...
	{
		Log::logMessage("Failed to create back buffer", pantheios::SEV_CRITICAL);
	...
	{
		Log::logMessage("Failed to create render target view", pantheios::SEV_CRITICAL);
	...
	{
		Log::logMessage("Failed to create depth stencil texture", pantheios::SEV_CRITICAL);
	...
	if (FAILED(mDevice->CreateDepthStencilView(mDepthStencil, &depthViewDesc,
		&mDepthStencilView)))
	{
		Log::logMessage("Failed to create depth stencil view", pantheios::SEV_CRITICAL);
...
HRESULT Dx11Renderer::compileShader(const char* fileName, const char* entryPoint,
	const char* shaderModel, ID3DBlob** blobOut)
{
	...
	if (FAILED(hResult))
	{
		if (errorBlob != nullptr)
		{
			...
			message << "Failed to compile shader " << fileName << ". Error message: "
		...
		{
			std::stringstream message;
			message << "Failed to compile shader " << fileName;
			Log::logMessage(message.str().c_str(), pantheios::SEV_ERROR);
		}
	}
