...
bsOctNode::bsOctNode(int id, const hkVector4& position, bsSceneGraph* sceneGraph,
	bsOctNode* parent, const hkAabb& aabb)
...
void bsOctNode::createChildren(unsigned short depth, bsSceneGraph* sceneGraph)
{
	++depth;
	mDepth = depth;
	...
	for (int x = 0; x < 2; ++x)
	{
		for (int y = 0; y < 2; ++y)
		{
			for (int z = 0; z < 2; ++z)
			{
				...
				bsOctNode* node = new bsOctNode(sceneGraph->getNumCreatedObjects(),
